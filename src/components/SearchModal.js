...
const SearchModal = (data, value) => {
  ...
  const normalizedValue = convertVi(lowerCasedValue);
  ...
  return data.filter(item =>
    Object.values(item).some(val => {
      ...
      const convertedVal = convertVi(lowerVal);

      return (
        convertedVal.includes(normalizedValue) ||
        lowerVal.includes(lowerCasedValue)
      );
    }),
  );
};
