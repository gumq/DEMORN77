...
const ProgressTab = ({ detailConfirmRequest }) => {
    const languageKey = useSelector(translateLang)
    const latestItem = detailConfirmRequest?.Progress?.reduce((latest, current) => {
        return moment(current.CreateDate).isAfter(moment(latest.CreateDate)) ? current : latest;
    }, detailConfirmRequest?.Progress?.[0]);
    ...
    return (
        <View style={stylesProgress.container}>
            {detailConfirmRequest?.Progress?.length > 0 ? (
                <FlatList
                    data={detailConfirmRequest?.Progress}
