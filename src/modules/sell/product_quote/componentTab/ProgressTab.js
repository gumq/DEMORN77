...
const ProgressTab = ({ detailProductQuote }) => {
    const languageKey = useSelector(translateLang)
    const latestItem = detailProductQuote?.Progress?.reduce((latest, current) => {
        return moment(current.CreateDate).isAfter(moment(latest.CreateDate)) ? current : latest;
    }, detailProductQuote?.Progress?.[0]);
    ...
    return (
        <View style={stylesProgress.container}>
            {detailProductQuote?.Progress?.length > 0 ? (
                <FlatList
                    data={detailProductQuote?.Progress}
