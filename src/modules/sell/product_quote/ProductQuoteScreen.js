...
const ProductQuoteScreen = () => {
  ...
  const {isSubmitting, listProductQuote} = useSelector(
  ...
  useEffect(() => {
    ...
    const unsubscribe = navigation.addListener('focus', () => {
      dispatch(fetchListProductQuote());
    });
    return unsubscribe;
  }, [navigation]);
  ...
  const onChangeText = textSearch => {
    if (textSearch?.length) {
      ...
      const resultsData = SearchModal(listProductQuote, textSearch);
      setSearchResults(resultsData);
    } else {
      setSearchResults(listProductQuote);
    }
  };

  const moveToDetailCusRequirement = item => {
    navigation.navigate(routes.DetailProductQuoteScreen, {item: item});
  };
          ...
          <View style={styles.bodyCard}>
            {item?.Customers ? (
              <View style={styles.containerBodyText}>
                ...
                <Text style={styles.contentBody}>{item?.Customers}</Text>
              </View>
            ) : null}
            {item?.GoodsTypes ? (
              <View style={styles.containerBodyText}>
                ...
                <Text style={styles.contentBody}>{item?.GoodsTypes}</Text>
              </View>
            ) : null}
  ...
  useEffect(() => {
    dispatch(fetchListCategoryTypeQuote());
    const bodyCustomer = {
  ...
  return (
    <LinearGradient
      ...
      <Button
        style={styles.btnAdd}
        onPress={() => navigation.navigate(routes.FormProductQuote)}>
...
export default ProductQuoteScreen;
