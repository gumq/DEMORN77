...
const DetailProductQuoteScreen = ({route, item}) => {
  ...
  const {isSubmitting, detailProductQuote} = useSelector(
    state => state.ProductQuote,
  );
  ...
  useEffect(() => {
    ...
    dispatch(fetchDetailProductQuote(body));
  }, [itemData]);

  const handleFormEdit = () => {
    navigation.navigate(routes.FormProductQuote, {
      item: detailProductQuote,
      editPrice: true,
    });
  };
  ...
  const submitApproval = async () => {
    const body = {
      dataJson: [
        {
          OID: itemData?.OID,
          FactorID: detailProductQuote?.FactorID,
          EntryID: detailProductQuote?.EntryID,
          ApprovalProcessID: detailProductQuote?.ApprovalProcessID,
  ...
  const appliedToArray =
    detailProductQuote?.AppliedToID?.split(',').map(Number);
  const isPermission =
    appliedToArray?.includes(userInfo?.UserID) &&
    detailProductQuote?.IsLock !== 0;
    ...
    <LinearGradient
      ...
      <SafeAreaView style={stylesDetail.container}>
        <HeaderBack
          ...
          btn={detailProductQuote?.IsLock === 0 ? true : false}
        ...
        <View style={stylesDetail.scrollView}>
          ...
          <ScrollView
            ...
            {selectedTab.id === 1 && (
              <DetailTab {...{detailProductQuote, itemData}} />
            )}
            {selectedTab.id === 2 && (
              <ProgressTab {...{detailProductQuote, itemData}} />
            )}
          </ScrollView>
        </View>
...
export default DetailProductQuoteScreen;
