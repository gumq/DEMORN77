...
const AccordingPlan = ({ searchResults, selectedItem, dateStates }) => {
    const dispatch = useDispatch();
    const navigation = useNavigation();
    const languageKey = useSelector(translateLang)
    const [isRefreshing, setRefresh] = useState(false);
    const data = searchResults?.filter(item => item?.VisitTypeCode === 'OFFRouteVisit' || item?.VisitTypeCode === 'UnplannedVisit')
