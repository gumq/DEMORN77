...
import {ApiCustomerProfiles_Add, ApiVisitForUsers_OFFRoute} from 'action/Api';
import {
  fetchListProvinceCity,
  fetchListWardCommune,
} from 'store/accCustomer_Profile/thunk';
import {fetchListCustomerOffRoute} from 'store/accVisit_Customer/thunk';

const NewCustomer = ({parentID, closeModal, reason}) => {
    ...
    async () => {
      ...
      try {
        ...
        if (customerData.StatusCode === 200 && customerData.ErrorCode === '0') {
          const createdCustomer = customerData?.Result[0];
          if (reason != true) {
            const bodyOffRoute = {
              ID: 0,
            ...
            const resOffRoute = await ApiVisitForUsers_OFFRoute(bodyOffRoute);
            const offRouteData = resOffRoute.data;

            if (
              offRouteData.StatusCode === 200 &&
              offRouteData.ErrorCode === '0'
            ) {
              dispatch(fetchListCustomerOffRoute());
              NotifierAlert(
                3000,
                languageKey('_notification'),
                offRouteData.Message,
                'success',
              );
              closeModal();
            } else {
