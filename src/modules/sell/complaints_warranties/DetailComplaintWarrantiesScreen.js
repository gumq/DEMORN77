...
import { View, StatusBar, ScrollView, TextInput, Text } from "react-native";
...
const DetailComplaintWarrantiesScreen = ({ route, item }) => {
    ...
    const { isSubmitting, detailComplaintWarraties } = useSelector(state => state.ComplaintWarranties);
    ...
    const submitApproval = async () => {
        const body = {
            dataJson: [
                {
                    OID: itemData?.OID,
                    FactorID: detailComplaintWarraties?.FactorID,
                    EntryID: detailComplaintWarraties?.EntryID,
                    ApprovalProcessID: detailComplaintWarraties?.ApprovalProcessID,
    ...
    const appliedToArray = detailComplaintWarraties?.AppliedToID?.split(",").map(Number);
    const isPermission = appliedToArray?.includes(userInfo?.UserID) && detailComplaintWarraties?.IsLock !== 0;
    ...
    return (
        <LinearGradient style={stylesDetail.container} start={{ x: 0.44, y: 0.45 }} end={{ x: 1.22, y: 0.25 }} colors={['#FFFFFF', '#FFFFFF',]} pointerEvents="box-none">
            ...
            <SafeAreaView style={stylesDetail.container}>
                <HeaderBack
                    title={detailComplaintWarraties?.OID}
                ...
                <View style={stylesDetail.scrollView}>
                    ...
                    >
                        {selectedTab.id === 1 && <DetailTab {...{ detailComplaintWarraties, itemData }} />}
                        {selectedTab.id === 2 && <ProgressTab {...{ detailComplaintWarraties, itemData }} />}
                    </ScrollView>
                </View>
                {/* <View style={stylesDetail.containerFooter}>
                    ...
                    >
                        <Text style={stylesDetail.txtBtnFooter}>{languageKey('_approve')}</Text>
                    </Button>
                </View> */}
                <Modal
                    useNativeDriver
                ...
                >
                    <View style={stylesDetail.optionsModalContainer}>
                        <Text style={stylesDetail.headerModalApproval}>{languageKey('_approval_information')}</Text>
                        ...
                        <View style={stylesDetail.containerFooterModal}>
                            ...
                            >
                                <Text style={stylesDetail.txtBtnFooterModal}>{languageKey('_confirm')}</Text>
                            </Button>
                        </View>
                    </View>
                </Modal>
...
export default DetailComplaintWarrantiesScreen;