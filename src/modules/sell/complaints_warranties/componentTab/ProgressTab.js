...
const ProgressTab = ({ detailCatalogue }) => {
    const languageKey = useSelector(translateLang)
    const latestItem = detailCatalogue?.Progress?.reduce((latest, current) => {
        return moment(current.CreateDate).isAfter(moment(latest.CreateDate)) ? current : latest;
    }, detailCatalogue?.Progress?.[0]);
    ...
    return (
        <View style={stylesProgress.container}>
            {detailCatalogue?.Progress?.length > 0 ? (
                <FlatList
                    data={detailCatalogue?.Progress}
