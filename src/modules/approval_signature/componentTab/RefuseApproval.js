...
const RefuseApproval = ({filterApprovalList}) => {
  const dispatch = useDispatch();
  const navigation = useNavigation();
  const languageKey = useSelector(translateLang);
  const [isRefreshing, setRefresh] = useState(false);
  const listRefuse = filterApprovalList.filter(
    approval => approval.Status === -1||approval.Status === '-1',
  );
      ...
      {listRefuse?.length > 0 ? (
        ...
        <ScrollView
          ...
          <View>
            ...
            <Text style={stylesAllApproval.txtContent}>
