...
const RejectedBySuperiors = ({ filterApprovalList }) => {
    const dispatch = useDispatch()
    const navigation = useNavigation();
    const languageKey = useSelector(translateLang)
    const [isRefreshing, setRefresh] = useState(false);
    const listApproved = filterApprovalList.filter((approval) => approval.Status === '2' || approval.Status === 2,)
...
export default RejectedBySuperiors;