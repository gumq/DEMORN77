...
const AllNotify = ({listNotify}) => {
  ...
  const handleDeleNotify = useCallback(
    async item => {
      try {
        ...
        console.log('NotifyScreen-handleDeleNotify', error);
      }
    },
    [dispatch, languageKey],
  );

  const handleViewNotify = useCallback(
    async item => {
      let data = {ListDetailID: String(item.DetailID)};

      if (item.IsView !== 1) {
        try {
          const result = await ApiUpdateViewNoti(data);
          if (result.data.ErrorCode === '0') {
            await dispatch(fetchTotalNotify());
            await dispatch(fetchListNotify());
          }
          ...
          console.log('NotifyScreen-handleViewNotify', error);
        }
      }
    ...
    [dispatch, navigation, languageKey],
  );

  const _keyExtractor = item => `${item.DetailID}`;
