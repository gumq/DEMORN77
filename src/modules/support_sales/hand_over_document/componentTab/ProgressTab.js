...
const ProgressTab = ({ detailHandOverDoc }) => {
    const languageKey = useSelector(translateLang)
    const latestItem = detailHandOverDoc?.Progress?.reduce((latest, current) => {
        return moment(current.CreateDate).isAfter(moment(latest.CreateDate)) ? current : latest;
    }, detailHandOverDoc?.Progress?.[0]);
    ...
    return (
        <View style={stylesProgress.container}>
            {detailHandOverDoc?.Progress?.length > 0 ? (
                <FlatList
                    data={detailHandOverDoc?.Progress}
