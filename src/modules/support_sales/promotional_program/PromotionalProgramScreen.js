...
const PromotionalProgramScreen = () => {
    ...
    const { isSubmitting, listPromotionPrograms } = useSelector(state => state.PromotionPrograms);

    useEffect(() => {
        LogBox.ignoreLogs(["VirtualizedLists should never be nested"])

        const unsubscribe = navigation.addListener('focus', () => {
            dispatch(fetchListPromotionPrograms())
        });
        return unsubscribe;
    }, [navigation])
    ...
    const onChangeText = textSearch => {
        if (textSearch?.length) {
            ...
            const resultsData = SearchModal(listPromotionPrograms, textSearch);
            setSearchResults(resultsData);
        } else {
            setSearchResults(listPromotionPrograms);
        }
    };

    const moveToDetailProgram = (item) => {
        navigation.navigate(routes.DetailPromotionProgramScreen, { item: item })
    }
            ...
            <Button onPress={() => moveToDetailProgram(item)}>
                <View style={styles.cardProgram}>
                    ...
                    <View style={styles.containerContent}>
                        <Text style={styles.headerProgram}>{item?.PromotionName}</Text>
                        ...
                        <View style={styles.timeProgram}>
                            <Text style={styles.txtHeaderTime}>{item?.ShortContent}</Text>
                        </View>
                    </View>
                </View>
            </Button>
    ...
    return (
        <LinearGradient style={styles.container} start={{ x: 0.44, y: 0.45 }} end={{ x: 1.22, y: 0.25 }} colors={['#FFFFFF', '#FFFFFF',]} pointerEvents="box-none">
            ...
            <SafeAreaView>
                <HeaderBack
                    title={languageKey('_promotional_program')}
...
export default PromotionalProgramScreen;