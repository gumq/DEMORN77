...
const ProgressTab = ({ detailGiftPrograms }) => {
    const languageKey = useSelector(translateLang)
    const latestItem = detailGiftPrograms?.Progress?.reduce((latest, current) => {
        return moment(current.CreateDate).isAfter(moment(latest.CreateDate)) ? current : latest;
    }, detailGiftPrograms?.Progress?.[0]);
    ...
    return (
        <View style={stylesProgress.container}>
            {detailGiftPrograms?.Progress?.length > 0 ? (
                <FlatList
                    data={detailGiftPrograms?.Progress}
