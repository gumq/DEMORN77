...
const ProgressTab = ({ detailShowroom }) => {
    const languageKey = useSelector(translateLang)
    const latestItem = detailShowroom?.Progress?.reduce((latest, current) => {
        return moment(current.CreateDate).isAfter(moment(latest.CreateDate)) ? current : latest;
    }, detailShowroom?.Progress?.[0]);
    ...
    return (
        <View style={stylesProgress.container}>
            ...
            {detailShowroom?.Progress?.length > 0 ? (
                <FlatList
                    data={detailShowroom?.Progress}
