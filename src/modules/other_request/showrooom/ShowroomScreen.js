...
import { fetchListShowroom } from "@store/accOther_Proposal/thunk";
...
const ShowroomScreen = () => {
    ...
    const { isSubmitting, listShowroom } = useSelector(state => state.OtherProposal);

    useEffect(() => {
        LogBox.ignoreLogs(["VirtualizedLists should never be nested"])

        const unsubscribe = navigation.addListener('focus', () => {
            dispatch(fetchListShowroom())
        });
        return unsubscribe;
    }, [navigation])
    ...
    const onChangeText = textSearch => {
        if (textSearch?.length) {
            ...
            const resultsData = SearchModal(listShowroom, textSearch);
            setSearchResults(resultsData);
        } else {
            setSearchResults(listShowroom);
        }
    };

    const moveToDetailProgram = (item) => {
        navigation.navigate(routes.DetailShowroomScreen, { item: item });
    }
    ...
    return (
        <LinearGradient style={styles.container} start={{ x: 0.44, y: 0.45 }} end={{ x: 1.22, y: 0.25 }} colors={['#FFFFFF', '#FFFFFF',]} pointerEvents="box-none">
            ...
            <Button style={styles.btnAdd}
                onPress={() => navigation.navigate(routes.FormShowroomScreen)}
            >
